...
public class P3A {
	...
	public static void main(String[] args){
		...
		// Workaround to run the DE with randomized MLPs
		MultilayerPerceptron[] MLPs = new MultilayerPerceptron[20];
		for(int i = 0; i < MLPs.length; i++) {
			mlp = new MultilayerPerceptron(nodeCounts, 0.5, 0.1, inputVectors, true);
			MLPs[i] = mlp;
		}
		...
		double errors = de.averageGenerationFitness(de.fitness(inputs, expected));
		int p = 0;
		do {
			System.out.println("Error = " + errors);
			p++;
			System.out.println("Evolving Generation" + p);
			de.evolveOneGeneration(inputs, expected);
			double newError = de.averageGenerationFitness(de.fitness(inputs, expected));
		...
		while(!de.terminate(errors) && NUMGENS != 10); 
		mlp = de.result(de.fitness(inputs, expected));
