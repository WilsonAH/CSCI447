 ...
 */
public class othermlp{
	/**
	 * Example of the network solving the XOR-problem
	 */
	public static void main(String[] args){
		othermlp mlp = new othermlp(2, 5, 1, 1, 1.0);
	...
	private static double[] points = new double[10001];
	 ...
	 */
	public othermlp(int input, int hidden, int output, int numberOfHiddenLayers, double learningrate){
	...
	private class Neuron {
		...
		public void test(){
			for(Synaps n : this.forbundetTil){
				if(this.act){
					n.getTil().input(othermlp.activation(this.sum)*n.getVægt());
				}
				else{
			...
			if(this.act){
				this.senesteoutput = othermlp.activation(this.sum);	
			}
			else{
